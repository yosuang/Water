...
export default function addDirExtension(pi: ExtensionAPI) {
	...
	pi.on("before_agent_start", async (event) => {
		if (addedDirs.size === 0) return undefined;
...
The user added these directories with /add-dir. Treat them as part of the active workspace and use absolute paths when reading, searching, editing, or running shell commands inside them:
...
`,
	...
	pi.registerCommand("add-dir", {
		...
		getArgumentCompletions: (prefix) => {
			...
			return [
				{ value: "--list", label: "--list", description: "Show added directories" },
				{ value: "--clear", label: "--clear", description: "Remove all added directories" },
				{ value: "--remove ", label: "--remove", description: "Remove one added directory" },
			];
		},
		handler: async (args, ctx) => {
			...
			if (command === "--list" || command === "list") {
				...
				sendContextMessage(pi, content, { directories: [...addedDirs] });
			...
			if (command === "--clear" || command === "clear") {
				...
				addedDirs.clear();
				pi.appendEntry(STATE_ENTRY_TYPE, { action: "clear", timestamp: Date.now() } satisfies StateEntry);
				updateStatus(ctx);
				const content = `Cleared additional working directories. Removed:\n${previous.map((dir) => `- ${dir}`).join("\n")}`;
				sendContextMessage(pi, content, { directories: [] });
			...
			if (command === "--remove" || command === "remove") {
				try {
					...
					pi.appendEntry(STATE_ENTRY_TYPE, { action: "remove", path: dir, timestamp: Date.now() } satisfies StateEntry);
					updateStatus(ctx);
					const content = `Removed ${dir} from additional working directories.`;
					sendContextMessage(pi, content, { path: dir, directories: [...addedDirs] });
					ctx.ui.notify(content, "info");
				} catch (error) {
			...
			try {
				...
				if (addedDirs.has(dir)) {
					const content = `${dir} is already an additional working directory.`;
					sendContextMessage(pi, content, { path: dir, directories: [...addedDirs] });
					ctx.ui.notify(content, "info");
					return;
				}
				...
				addedDirs.add(dir);
				pi.appendEntry(STATE_ENTRY_TYPE, { action: "add", path: dir, timestamp: Date.now() } satisfies StateEntry);
				updateStatus(ctx);

				const content = `Added ${dir} as an additional working directory for this session.`;
				sendContextMessage(pi, content, { path: dir, directories: [...addedDirs] });
				ctx.ui.notify(`${content} It will be included in future LLM context.`, "info");
			} catch (error) {
				ctx.ui.notify(error instanceof Error ? error.message : String(error), "warning");
			}
		},
